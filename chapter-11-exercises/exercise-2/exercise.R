...
two_wheel_20_mi <- filter(vehicles, drive == "2-Wheel Drive", cty > 20)

# Of the above vehicles, what is the vehicle ID of the vehicle with the worst 
# hwy mpg?
# Hint: filter for the worst vehicle, then select its ID.
filter <- filter(two_wheel_20_mi, hwy == min(hwy))
worst_id <- select(filter, id)
print(worst_id)
...
function(year_choice, make_choice) {
  ...
  filtered <- (filtered, hwy == max(hwy))
  select <- select(filtered, model)
  return(select)
}
